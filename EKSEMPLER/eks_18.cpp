...
/**
 *  Type (med 'Telt', 'B(o)bil', 'C(a)mpingvogn').
 */
enum Type { Telt, Bbil, Cmpingvogn };
...
class Enhet  {             //  = TELT også !!!
  ...
  protected:
    Type type;             //  Type enhet:  'Telt', 'Bbil', 'Cmpingvogn'
...
class Bobil : public Enhet {
  ...
  public:
    ...
    Bobil(int nr) : Enhet(nr)      //  Setter initielle data:
                    {  strom = false;   type = Bbil;   grunnpris = BIL;  }
...
public:
    ...
    Campingvogn(int nr) : Bobil(nr)  {  fortelt = false;   lengde = 0.0F;
                      type = Cmpingvogn;   grunnpris = VOGN;  }
...
void Enhet::skrivType() const {
    switch (type) {
      ...
      case Bbil:        cout << "Bobil";        break;
      case Cmpingvogn:  cout << "Campingvogn";  break;
    }
}
