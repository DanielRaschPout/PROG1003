...
class Lege {
  ...
  public:
      Lege(string nvn)  {  navn = nvn;  tlf = 0;  }
      ~Lege();
      string hentID()   {  return navn;  }
...
void lesLegerOgResepterFraFil() {
    ...
    if (innfil) {                                  //  Filen funnet:
        ...
        for (int i = 1;  i <= antLeger;  i++) {    //  Leser alle legene:
            ...
            lege = new Lege(lNavn);                //  Lager nytt Lege-objekt.
...
void pasientOversikt() {
    ...
    if ((pasNr = gPasienter.finnes(pasNavn)) != 0)  //  Navnet finnes:
        for (const auto & val : gLeger)             //  Hver lege skriver selv
...
void registrerResept() {
    ...
    if (nr != 0) {                           //  Blant de 20x aktuelle:
        ...
        if (gLeger.find(legeNavn) == gLeger.end()) {   //  Nytt legenavn:
            lege = new Lege(legeNavn);       //  Ny lege opprettes,
...
//             [] (const auto & val) { (val.second)->skrivHovedData();  } );
    else
...
void skrivLegerOgResepterTilFil() {
    ...
    for (const auto & val : gLeger)        //  Hver lege skriver selv til fil:
